...
class RoombaComponent : public UARTDevice, public CustomAPIDevice, public PollingComponent {
 ...
 private:
  ...
  RoombaComponent(uint8_t brcPin, UARTComponent *parent, uint32_t updateInterval, bool lazy650Enabled)
      : UARTDevice(parent), PollingComponent(updateInterval) {
    this->brcPin = brcPin;
  ...
  void start_oi() { write(StartCmd); }

  void reset() { write(ResetCmd); }
  ...
  void clean() { write(CleanCmd); }

  void dock() { write(DockCmd); }

  void spot() { write(SpotCmd); }

  void sleep() { write(PowerCmd); }
  ...
  std::string get_activity(uint8_t charging, int16_t current) {
    bool isCharging = charging == ChargeStateReconditioningCharging || charging == ChargeStateFullCharging ||
                      charging == ChargeStateTrickleCharging;
